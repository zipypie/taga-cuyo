...
class LessonProgressService {
  ...
  Future<int> getCompletedLessons(String userId) async {
    DocumentReference userLessonProgressDoc =
        _firestore.collection('userLessonProgress').doc(userId);

    DocumentSnapshot snapshot = await userLessonProgressDoc.get();

    if (snapshot.exists) {
      Map<String, dynamic> userLessonProgressData =
    ...
    try {
      ...
      if (userProgressDoc.exists) {
        return (userProgressDoc.data()
                    as Map<String, dynamic>?)?['lessons_progress']?[documentId]
                ?['isCompleted'] ??
            false;
      } else {
  ...
  Future<bool> isLastDocument(String documentId, String userId) async {
    try {
      // Reference the lessons collection to count all lessons
      CollectionReference lessonsCollection = _firestore.collection('lessons');
      QuerySnapshot lessonsSnapshot = await lessonsCollection.get();
      int totalLessonsCount = lessonsSnapshot.docs.length;

      // Get the count of completed lessons for the user
      int completedLessonsCount = await getCompletedLessons(userId);

      // Check if the current lesson is the last to be completed
      return (completedLessonsCount == totalLessonsCount - 1);
    } catch (e) {
      Logger.log('Error checking if last document: $e');
      return false; // Return false in case of error
    }
  }
}
